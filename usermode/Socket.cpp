...
uint64_t driver::get_process_client_address(const SOCKET connection, const uint32_t process_id)
{
	Packet packet{ };

	packet.header.magic = packet_magic;
	packet.header.type = PacketType::packet_get_client_address;

	auto& data = packet.data.get_client_address;
...
uint64_t driver::get_process_engine_address(const SOCKET connection, const uint32_t process_id)
{
	Packet packet{ };

	packet.header.magic = packet_magic;
	packet.header.type = PacketType::packet_get_engine_address;
